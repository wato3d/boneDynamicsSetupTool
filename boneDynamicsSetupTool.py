...
def boneDynamicsCreate():
    ...
    cmds.parentConstraint(joints[0], AddiGrp, mo=True)
...
def create_dynamics_chain(constraint=True):
    ...
    if not dynamicsName:
        dynm_attr_long = "dynamicsIntencity01"
        dynm_attr_nice = "Dynamics Intencity01"
        cnt = 1
        while cmds.attributeQuery(dynm_attr_long, node=attributeObj, exists=True):
            cnt += 1
            dynm_attr_long = "dynamicsIntencity" + f"{cnt:02d}"
            dynm_attr_nice = "Dynamics Intencity" + f"{cnt:02d}" 
    else:
        dynm_attr_long = dynamicsName + "dynamicsIntencity01"
        dynm_attr_nice = dynamicsName + "Dynamics Intencity01"
        cnt = 1
        while cmds.attributeQuery(dynm_attr_long, node=attributeObj, exists=True):
            cnt += 1
            dynm_attr_long = dynamicsName + "dynamicsIntencity" + f"{cnt:02d}"
            dynm_attr_nice = dynamicsName + "Dynamics Intencity" + f"{cnt:02d}"
    ...
    if constraint:
        for i, (node, jnt) in enumerate(joint_list):
            ...
            cmds.connectAttr(attributeObj + "." + dynm_attr_long, '{}.weight'.format(pb), f=True)
            for at in 'XYZ':
                cmds.connectAttr(
...
def create_attribute(
    ...
    cmds.addAttr(
        attr_ctl, 
        longName=dynm_attr_long,
        niceName=dynm_attr_nice,
        attributeType="bool",
        defaultValue=True,
        keyable=True
        )
